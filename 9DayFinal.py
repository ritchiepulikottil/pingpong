...
class PongPaddle(Widget):
    ...
    def bounce_ball(self,ball):
        if self.collide_widget(ball): #collision detection method inside KivY
            ball.velocity_x *= -1
...
class PongGame(Widget):
    ...
    def update(self,dt): #called 60 times in one second
                ...
                #bouncing off @left and increase score
                if  self.ball.x<0 :
                    self.ball.velocity_x *= -1
                    self.player1.score +=1
                    
                #bouncing off @right and increase score
                if self.ball.x>self.width-50:  #-50 cuz half of the ball dissappears @right
                    self.ball.velocity_x *= -1
                    self.player2.score +=1
                


                #to check if ball collides on paddle continuously. hence put inside update method
                self.player1.bounce_ball(self.ball)
                self.player2.bounce_ball(self.ball)

                

    def on_touch_move(self,touch): #when the player clicks the mouse and moves
